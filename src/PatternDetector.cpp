...
PatternDetector::PatternDetector(cv::Ptr<cv::Feature2D> detector, 
    ...
    , homographyReprojectionThreshold(1)
{
}
...
void PatternDetector::train(const Pattern& pattern)
{
    ...
    cv::Mat descriptor;
    pattern.descriptors.convertTo(descriptor,CV_32F);
...
void PatternDetector::buildPatternFromImage(const cv::Mat& image, Pattern& pattern) const
{
    ...
    pattern.points3d[0] = cv::Point3f(-2, -2, 0);
    pattern.points3d[1] = cv::Point3f( 2, -2, 0);
    pattern.points3d[2] = cv::Point3f( 2, 2, 0);
    pattern.points3d[3] = cv::Point3f(-2, 2, 0);
...
// #endif
            ...
            if (!extractFeatures(m_warpedImg, warpedKeypoints, m_queryDescriptors))
                return true;
