 ...
 */
int append_image_data_csv( char *filename,const char *image_filename, int image_data, int reset_file ) {
  char buffer[256];
  char mode[8];
  FILE *fp;
  ...
  std::string s = std::to_string(image_data);
...
int read_image_data_csv( char *filename, std::vector<char *> &filenames, std::vector<int> &data, int echo_file ) {
  FILE *fp;
  int ival;
  char img_file[256];
